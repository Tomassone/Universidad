function parsificaJSON(jsonText) {
	...
	for (var a = 0, b = items.length; a < b; a++) {
		...
		itemNodes[a].idDrink = items[a].idDrink;
		itemNodes[a].stato = items[a].stato;
		itemNodes[a].costo = items[a].costo;
		itemNodes[a].unita = items[a].unita;
	}
	...
	risultato = "<form action='order.jsp'><p>"

	for(var c = 0; c < itemNodes.length; c++) {
		risultato += "<input type='checkbox'" + "name=" + "'" + itemNodes[c].idDrink + "'" + "value=" + "'" + itemNodes[c].idDrink  + "'" + ">" + itemNodes[c].idDrink + "<br/>";
	};

	// chiudiamo il form creato
	risultato += '</p><p><input type="submit" value="Ordina">	<input type="reset" value="Azzera">' + ' </p></form>';
