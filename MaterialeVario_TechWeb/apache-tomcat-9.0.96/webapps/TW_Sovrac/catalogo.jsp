...
void remove(Cart cart, Prodotto item) { 
	cart.remove(item); 
} 
%>
...
<jsp:useBean id="catalogue" class="Beans.Catalogo" scope="session" /> 
...
else if(order == item.getQuantity()) 
{   remove(cart,item);  } 
add(cart,item,order); 
} 
...
Prodotto[] cartItems = cart.getItems().toArray(new Prodotto[0]); 
...
<br/> 
		...
		<a href="?empty=ok">Remove all items from the cart</a> 
		<form action="Finalizza" method="get"> 
		<input type="submit" name="finalizza" value="Finalizza Ordine"/> 
		</form> 
